...
export class Blocked extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      displayBlockedLink:
        props.displayBlockedLink !== undefined ? props.displayBlockedLink : true,
    };
  ...
  componentDidMount() {
    ...
    console.log(
      '[Blocked] Has custom message from URL:',
      hasCustomMessageFromUrl,
      customMessage,
    );
    ...
    storage
      ...
      .then((items) => {
        ...
        if (items.blockTab && typeof items.blockTab === 'object') {
          ...
          showBlockedLink =
            items.blockTab.displayBlockedLink !== undefined
              ? items.blockTab.displayBlockedLink
              : true;
        } else {
          storageMessage = items.message || '';
          showBlockedLink =
            items.displayBlockedLink !== undefined ? items.displayBlockedLink : true;
        }
        ...
        this.setState({
          ...
          message: this.state.hasCustomMessageFromUrl
            ...
            : storageMessage.length
              ? storageMessage
              : translate('defaultBlockingMessage'),
        });

        console.log(
          '[Blocked] Final message decision:',
          this.state.hasCustomMessageFromUrl
            ? 'Using URL message'
            ...
            <div className="distract-cursor distract-select distract-info-container">
              ...
              {isDevEnv && (
                ...
                >
                  {' '}
                  <p>
                    <strong>URL Hash:</strong> {this.state.hash}
                  </p>
                  <p>
                    <strong>Parsed URL:</strong> {this.state.url}
                  </p>
                  <p>
                    <strong>Parsed Reason:</strong> {this.state.reason}
                  </p>
                  <p>
                    <strong>Message from URL:</strong>{' '}
                    {this.state.hasCustomMessageFromUrl ? 'Yes' : 'No'}
                  </p>
                  <p>
                    <strong>Message being displayed:</strong> {blockMessage}
                  </p>
                </div>
              )}
            </div>
