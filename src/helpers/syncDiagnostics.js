  ...
  async recordSyncError(error, operation = 'general') {
    try {
      ...
      const updatedStatus = {
        ...
        consecutiveErrors,
        syncHealth:
          consecutiveErrors >= 3 ? 'poor' : consecutiveErrors >= 1 ? 'fair' : 'good',
      };
...
export const diagnostics = {
    ...
    try {
      ...
      if (results.syncAvailable) {
        try {
          ...
          ['blacklist', 'whitelist', 'blacklistKeywords', 'whitelistKeywords'].forEach(
            (key) => {
  ...
  async testSync() {
    ...
    try {
      ...
      results.steps.push(
        `✅ Current sync storage: ${preTestTelemetry.bytesUsed} bytes (${preTestTelemetry.quotaPercentage}% of quota)`,
      );
      ...
      results.steps.push(
        `✅ Found ${effectiveSettings.totalRules} blocking rules, ${effectiveSettings.totalKeywords} keywords across ${effectiveSettings.enabledCategories} enabled categories`,
      );
      ...
      results.telemetry.performance.writeLatency = Math.round(
        writeEndTime - writeStartTime,
      );
      results.steps.push(
        `✅ Successfully wrote to sync storage (${results.telemetry.performance.writeLatency}ms)`,
      );
      ...
      results.steps.push(
        `✅ Successfully read from sync storage (${results.telemetry.performance.readLatency}ms)`,
      );
      ...
      if (!this.isDataIntegrityValid(testData, readData[testId])) {
        throw new Error(
          'Data integrity check failed - written and read data do not match',
        );
      }
      ...
      if (chrome.storage && chrome.storage.onChanged) {
        ...
        results.telemetry.performance.listenerLatency = Math.round(
          listenerEndTime - listenerStartTime,
        );

        if (listenerTriggered) {
          results.steps.push(
            `✅ Storage change listener working correctly (${results.telemetry.performance.listenerLatency}ms)`,
          );
        } else {
          results.steps.push(
            `⚠️ Storage change listener may not be working (${results.telemetry.performance.listenerLatency}ms)`,
          );
        }
      } else {
      ...
      results.steps.push(
        `✅ Post-test sync storage: ${postTestTelemetry.bytesUsed} bytes (${postTestTelemetry.quotaPercentage}% of quota)`,
      );
  ...
  async forceSyncAllData() {
    try {
      ...
      Object.keys(localData).forEach((key) => {
        if (
          !localOnlySettings.some(
            (setting) => setting === key || key.startsWith(setting + '.'),
          )
        ) {
  ...
  async diagnoseProblems() {
    ...
    try {
      ...
      if (storageTelemetry.quotaPercentage > 85) {
        problems.push(
          `Sync storage critical: ${storageTelemetry.quotaPercentage}% of quota used (${storageTelemetry.bytesUsed}/${storageTelemetry.quotaBytes} bytes)`,
        );
        suggestions.push(
          'Urgent: Clear old data or reduce rule/keyword lists to prevent sync failures',
        );
      } else if (storageTelemetry.quotaPercentage > 70) {
        problems.push(
          `Sync storage warning: ${storageTelemetry.quotaPercentage}% of quota used (${storageTelemetry.bytesUsed}/${storageTelemetry.quotaBytes} bytes)`,
        );
      ...
      if (storageTelemetry.largestItems && storageTelemetry.largestItems.length > 0) {
        ...
        if (largestItem.size > 10000) {
          // >10KB for single item
          problems.push(
            `Large storage item detected: "${largestItem.key}" uses ${largestItem.size} bytes`,
          );
          suggestions.push(
            `Review and optimize the "${largestItem.key}" list to reduce storage usage`,
          );
        }
      }
      ...
      if (effectiveSettings.totalRules === 0 && effectiveSettings.totalKeywords === 0) {
        if (effectiveSettings.isEnabled && effectiveSettings.syncEnabled) {
          ...
          if (telemetry.storage && telemetry.storage.bytesUsed < 100) {
            // Very minimal storage suggests fresh install
            suggestions.push(
              'Add blocking rules or keywords to enable content filtering functionality',
            );
          } else {
            problems.push(
              'No active blocking rules or keywords found despite extension being enabled',
            );
            suggestions.push(
              'Check if rules failed to sync properly or add new blocking rules',
            );
          }
        }
        // If extension is disabled, don't warn about missing rules
      } else if (
        effectiveSettings.enabledCategories === 0 &&
        (effectiveSettings.totalRules > 0 || effectiveSettings.totalKeywords > 0)
      ) {
      ...
      if (
        effectiveSettings.totalRules > 1000 &&
        telemetry.storage?.quotaPercentage > 50
      ) {
        problems.push(
          `Large rule set: ${effectiveSettings.totalRules} rules using ${telemetry.storage.quotaPercentage}% of storage quota`,
        );
        suggestions.push(
          'Consider optimizing rules or using more specific patterns to reduce storage usage',
        );
      } else if (effectiveSettings.totalRules > 2000) {
        // Only warn at very high counts regardless of storage
        problems.push(
          `Very large rule set: ${effectiveSettings.totalRules} rules across ${effectiveSettings.enabledCategories} categories`,
        );
        suggestions.push(
          'Consider if all rules are necessary - extremely large rule sets may impact performance',
        );
      }
      ...
      if (!effectiveSettings.syncEnabled) {
        ...
        suggestions.push(
          'Enable sync in extension settings to synchronize rules across devices',
        );
      }
      ...
      if (chrome?.storage?.sync?.get && chrome?.storage?.local?.get) {
        try {
          ...
          if (syncData && localData) {
            ...
            if (duplicates > 5) {
              problems.push(
                `Data synchronization inefficiency: ${duplicates} settings are stored in both local and cloud storage, potentially causing conflicts and using extra storage space`,
              );
              suggestions.push(
                'Run storage cleanup to remove local duplicates and improve sync reliability - the extension will use cloud storage as the primary source',
              );
            ...
            const missingTimestamps = timestampKeys.filter(
              (key) => !syncData[key] && !localData[key],
            );
            if (missingTimestamps.length > 0) {
              problems.push(`Missing sync timestamps: ${missingTimestamps.join(', ')}`);
              suggestions.push(
                'Timestamps help resolve sync conflicts - consider forcing a sync to generate them',
              );
            }
            ...
            ['blacklist', 'whitelist', 'blacklistKeywords', 'whitelistKeywords'].forEach(
              (key) => {
                ...
                if (Array.isArray(effectiveArray) && effectiveArray.length > 0) {
                  ...
                  if (arraySize > 10240) {
                    problems.push(
                      `Large ${key}: ${effectiveArray.length} items using ${Math.round(arraySize / 1024)}KB of storage`,
                    );
                    suggestions.push(
                      `Consider optimizing ${key} patterns or removing unused entries`,
                    );
                  }
                  // Or if array count is extremely high (>1000 items)
                    ...
                    suggestions.push(
                      `Consider organizing ${key} into more specific patterns`,
                    );
                  }
                }
              },
        ...
        try {
          if (localOnlySettings.includes(key)) {
            ...
            results.cleanedUp.push(
              `Removed ${key} from sync storage (local-only setting)`,
            );
          } else {
            // Remove from local storage if it's a syncable setting
            await chrome.storage.local.remove(key);
            results.cleanedUp.push(
              `Removed ${key} from local storage (syncable setting)`,
            );
          }
    ...
    try {
      ...
      for (const key of arrayKeys) {
        if (syncData[key] && Array.isArray(syncData[key])) {
          ...
          analysis.emptyItems = array.filter(
            (item) => !item || item.trim() === '',
          ).length;
          ...
          const duplicateSavings =
            analysis.duplicates * JSON.stringify(array[0] || '').length;
      ...
      if (results.potentialSavings > 1000) {
        results.recommendations.push(
          `Potential space savings: ${results.potentialSavings} bytes`,
        );
      }

      const highPriorityIssues = results.analyzed.filter(
        (a) => a.priority === 'high',
      ).length;
      if (highPriorityIssues > 0) {
        results.recommendations.push(
          `${highPriorityIssues} arrays need immediate attention`,
        );
      }

      results.success = true;
      results.details.push(
        `Analyzed ${results.arraysAnalyzed} arrays, total size: ${results.totalSize} bytes`,
      );
    } catch (error) {
    ...
    try {
      ...
      standardLists.forEach(({ key, type }) => {
        ...
        if (Array.isArray(effectiveList) && effectiveList.length > 0) {
          const activeItems = effectiveList.filter(
